...
#CENÁRIO APENAS COM PEQUENOS CLUSTERS DE CONLUIO
ecomony <-
  list.files(path = "../economy_simulation/output-cluster1/", pattern = "data*.csv", full.names = TRUE) %>% 
...
ggplot(data=ecomony_2, aes(y=acertou_linear*100,x=honestidade*100,ymax = if_else(acertou==1,acertou*100,acertou*97.5), ymin = if_else(acertou*102.5+2.5<100,if_else(acertou==0,0,acertou*102.5+2.5),100),  fill =profile))  +
  ...
  #geom_text(aes(label=if_else(profile==" HS" | profile=="A" | profile=="C" | profile=="D" | profile=="E" | profile=="F" | profile=="G" | profile=="H" | profile=="I",format(round(acertou*100, 1), nsmall = 1),"")  ),  vjust=-0.1, color="black", size=3.5)+
